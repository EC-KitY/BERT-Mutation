...
def main():
    ...
    function_set_names = np.array([
        f.__name__ for f in function_set
    ])
    ...
    function_mapping = {
        f.__name__: f for f in function_set
    }

    terminals_mappings = {
        var: i for i, var in enumerate(terminal_set)
    }

    evaluator_class = SymbolicRegressionEvaluator()

    bert_model = BertMutation(
        operators_list=function_set_names,
        constant_names=terminal_set,
        get_fitness_func=evaluator_class.evaluate_individual,
        context_size=256, word_embedding_dim=20, n_layers=1, n_attention_heads=1,
